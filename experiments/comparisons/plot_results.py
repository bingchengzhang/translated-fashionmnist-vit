...
def _plot_training_dynamics(
    ...
    figure, axes = plt.subplots(2, 3, figsize=(13, 7.2), sharex=True, sharey=True)
    for axis, config_id in zip(axes.flat, config_ids):
        for train_mode, color in (("A", "#4C72B0"), ("B", "#DD8452")):
            history_path = (
                runs_dir / config_id / f"train_{train_mode}" / "history.csv"
            )
            with history_path.open(newline="", encoding="utf-8") as file:
                history = list(csv.DictReader(file))
            epochs = [int(row["epoch"]) for row in history]
            validation = [100 * float(row["val_accuracy"]) for row in history]
            axis.plot(
                epochs,
                validation,
                marker="o",
...
def generate_visualizations(
    ...
    _plot_training_dynamics(
        rows,
        Path(results_csv).parent / "runs",
