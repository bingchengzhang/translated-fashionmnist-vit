...
python -m optional_experiments_vs_teammate.run_comparisons \
  ...
  --num-workers 0 \
  --output-dir optional_experiments_vs_teammate/results_smoke
```
...
- `results/all_results.csv`：所有配置的四 setting 结果。
- `results/manifest.json`：环境、参数、随机种子与运行配置。
- `results/runs/`：训练历史、曲线、最佳 checkpoint 与元数据。
- `assets/model_comparison.png`：MLP/CNN/ViT 对比。
- `assets/patch_size_comparison.png`：patch size 对比。
- `assets/patch_embedding_comparison.png`：patch embedding 对比。
- `assets/position_generalization.png`：固定位置训练后的跨位置性能下降。
- `assets/teammate_baseline_comparison.png`：与同学仓库公开记录的参考对比。
- `assets/training_time_comparison.png`：统一实验协议下训练 A、B 两个模型的总耗时。
- `RESULTS.md`：可直接整理进实验报告的结果表和位置泛化数据。
...
`teammate_baseline.csv` 只摘录该版本 `results/*/summary.json` 中公开的四项
reported best accuracy。由于对方记录使用测试集逐 epoch 评价并保留最佳值，
该数据在图中作为外部参考展示，不作严格统计等价比较。

## 结果摘要

完整数值见 [RESULTS.md](RESULTS.md)。本次单随机种子实验有以下现象：
...
![MLP、CNN 与 ViT 对比](assets/model_comparison.png)

![ViT patch size 对比](assets/patch_size_comparison.png)

![Patch embedding 对比](assets/patch_embedding_comparison.png)

![位置泛化下降](assets/position_generalization.png)

![训练耗时](assets/training_time_comparison.png)

![同学项目外部参考](assets/teammate_baseline_comparison.png)
